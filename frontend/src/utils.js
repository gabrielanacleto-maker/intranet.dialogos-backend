...
export const ALLOWED_EMBED_DOMAINS = ['instagram.com', 'tiktok.com'];
export const TRUSTED_EMBED_DOMAINS = {
  'instagram.com': (url) => {
    const clean = url.split('?')[0].replace(/\/$/, '');
    return `<div style="padding:8px 0;max-width:400px;margin:0 auto"><iframe src="${clean}/embed" width="100%" height="480" frameborder="0" scrolling="no" allowtransparency loading="lazy" style="border-radius:12px;border:1px solid var(--border);display:block"></iframe></div>`;
  },
  'tiktok.com': (url) => {
    const clean = url.split('?')[0];
    return `<div style="padding:8px 0;max-width:400px;margin:0 auto"><iframe src="${clean}" width="100%" height="580" frameborder="0" allow="autoplay" loading="lazy" style="border-radius:12px;border:1px solid var(--border);display:block"></iframe></div>`;
  }
};

export function isValidEmbedDomain(url) {
  try {
    const u = new URL(url);
    return ALLOWED_EMBED_DOMAINS.some(d => u.hostname === d || u.hostname.endsWith('.' + d));
  } catch { return false; }
}

export const ROLE_CATEGORIES = {
  diretoria: {
    keywords: ['diretor', 'diretora', 'ceo', 'executivo', 'gestão estratégica', 'gestao estrategica'],
    class: 'role-diretora',
    glow: 'role-glow-rosa',
    style: { color: '#ff2d7a', textShadow: '0 0 8px rgba(255,45,122,0.8)', fontWeight: 700 },
  },
  rh: {
    keywords: ['rh', 'recursos humanos'],
    class: '',
    glow: 'role-glow-roxo',
    style: { color: '#f472b6', textShadow: '0 0 6px rgba(244,114,182,0.7)', fontWeight: 600 },
  },
  recepcao: {
    keywords: ['recepcion', 'recepcao', 'receção'],
    class: 'role-recepcionista',
    glow: 'role-glow-azul',
    style: { color: '#4fc3f7', textShadow: '0 0 6px rgba(79,195,247,0.6)', fontWeight: 600 },
  },
  financeiro: {
    keywords: ['financeiro', 'financeira', 'faturista', 'faturamento'],
    class: 'role-financeiro',
    glow: 'role-glow-laranja',
    style: { color: '#ff8c00', textShadow: '0 0 6px rgba(255,140,0,0.6)', fontWeight: 600 },
  },
  saude: {
    keywords: ['médico', 'medico', 'enfermagem', 'enfermeiro', 'radiologia', 'fisioterapeuta', 'biomédico', 'biomedico', 'nutricionista', 'psicólogo', 'psicologo', 'terapeuta ocupacional', 'técnica de enfermagem', 'tecnico de enfermagem', 'técnico de radiologia', 'tecnico de radiologia'],
    class: '',
    glow: 'role-glow-verde',
    style: { color: '#66bb6a', textShadow: '0 0 6px rgba(102,187,106,0.6)', fontWeight: 600 },
  },
  analistas: {
    keywords: ['analista', 'qualidade', 'qualidade'],
    class: '',
    glow: 'role-glow-azul',
    style: { color: '#4fc3f7', textShadow: '0 0 6px rgba(79,195,247,0.6)', fontWeight: 600 },
  },
  juridico: {
    keywords: ['advogado', 'advogada', 'jurídico', 'juridico'],
    class: '',
    glow: 'role-glow-roxo',
    style: { color: '#b39ddb', textShadow: '0 0 6px rgba(120,100,200,0.6)', fontWeight: 600 },
  },
};

export function getRoleCategory(role = '') {
  const r = role.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  for (const [cat, config] of Object.entries(ROLE_CATEGORIES)) {
    for (const kw of config.keywords) {
      if (r.includes(kw)) return cat;
    }
  }
  return null;
}
...
export function getRoleBadgeClass(role = '') {
  ...
  if (!cat) return '';
  return ROLE_CATEGORIES[cat].class;
}

export function getRoleGlowClass(role = '', dept = '') {
  const combined = (role + ' ' + dept).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const cat = getRoleCategory(combined);
  if (cat) return ROLE_CATEGORIES[cat].glow;
  if (combined.includes('líder') || combined.includes('lider') || combined.includes('liderança')) return 'role-glow-roxo';
  if (combined.includes('limpeza') || combined.includes('saúde') || combined.includes('saude') || combined.includes('enferm') || combined.includes('psicol') || combined.includes('nutrici')) return 'role-glow-verde';
  return '';
}

export function getRoleClass(role = '') {
  const cat = getRoleCategory(role);
  if (cat) return ROLE_CATEGORIES[cat].class || '';
...
export function getRoleStyle(role, is_rh, is_admin) {
  ...
  const cat = getRoleCategory(role);
  if (cat) {
    const cfg = ROLE_CATEGORIES[cat];
    if (cat === 'rh') {
      return is_rh ? cfg.style : { color: '#f472b6', textShadow: '0 0 6px rgba(244,114,182,0.7)', fontWeight: 600 };
    }
    return cfg.style;
  }
...
export function buildEmbedHtml(url) {
  ...
  if (isValidEmbedDomain(url)) {
    for (const [domain, builder] of Object.entries(TRUSTED_EMBED_DOMAINS)) {
      if (url.includes(domain)) return builder(url);
    }
  }

  // Instagram (old-style embed fallback)

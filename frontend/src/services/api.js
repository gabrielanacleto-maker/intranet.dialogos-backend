...
function getErrorMessage(err, fallback = 'Erro na requisição') {
...
export const api = {
  ...
  updateAboutMe: (about_me) => req('PATCH', '/api/users/me/about', { about_me }),
  ...
  uploadPostVideo: (file) => {
    const fd = new FormData(); fd.append('file', file);
    return req('POST', '/api/posts/upload-video', fd, true);
  },
  ...
  searchGifs: (q) => req('GET', `/api/giphy/search?q=${encodeURIComponent(q)}`),

  // Mood
  saveMood: (data) => req('POST', '/api/mood', data),
  getMoodHistory: () => req('GET', '/api/mood/history'),
  downloadMoodPdf: async () => {
    const headers = {};
    const token = getToken();
    if (token) headers['Authorization'] = `Bearer ${token}`;
    const res = await fetch(`${BASE}/api/mood/report.pdf`, { headers });
  ...
  downloadPriceTable: async (folderId) => {
    ...
    if (!res.ok) {
      ...
      throw new Error(getErrorMessage(err, 'Erro ao baixar tabela de preços'));
    }
    return res.blob();
  },

// Gestor e Equipe
  getGestores: () => req('GET', '/api/users/gestores'),
  getUserManager: (key) => req('GET', `/api/users/${key}/manager`),
  getUserTeam: (key) => req('GET', `/api/users/${key}/team`),
  assignManager: (data) => req('PUT', '/api/users/manager/assign', data),
  setOrgPosition: (data) => req('PUT', '/api/users/org-position', data),

...
import {
  ...
  ThemeBanner,
  ThemeAnimations,
  RomanticAvatarDecoration
} from './pages/TemasComemorativos';

export default function App() {
  const { user, mustChangePassword, logout, canSeeNovidades, canAdmin, checkingAuth } = useAuth();
  const getPageFromPath = () => {
    const match = window.location.pathname.match(/^\/colleague\/([^/]+)$/);
    return match ? `colleague:${decodeURIComponent(match[1])}` : 'feed';
  };
  const [page, setPage] = useState(getPageFromPath);

  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('dialogos_theme') === 'dark');

  // Apply theme + comemorativo
  useEffect(() => {
    document.body.classList.toggle('dark-theme', darkMode);
    localStorage.setItem('dialogos_theme', darkMode ? 'dark' : 'light');
    const theme = getActiveTheme();
    document.querySelectorAll('[class*="tema-"]').forEach(c => document.body.classList.remove(c));
    if (theme) document.body.classList.add(theme.bgClass);
  }, [darkMode]);
  ...
  function changePage(newPage) {
    if (!String(newPage).startsWith('colleague:')) window.history.pushState({}, '', '/');
    setPage(newPage);
  }
  ...
  function renderPage() {
    if (page.startsWith('colleague:')) return <ColleagueProfilePage profileKey={page.slice('colleague:'.length)} />;
    switch (page) {
  ...
  return (
    <>
      <CursorGlow />
      <ThemeAnimations />
      {activeTheme && <ThemeBanner />}
      <FloatingHearts />
      {activeTheme && <ThemeDecorations density={16} />}
      <div className="app-layout">
        {/* TOPBAR */}
        <header className="topbar">
          <div className="topbar-logo">
            <img
              className="dialogos-logo-spin"

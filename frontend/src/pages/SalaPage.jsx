          ...
          <>
            ...
            {activeTab === 'members' && (
              <div className="surface-card" style={{ padding: 14 }}>
                ...
                <div style={{ display: 'grid', gap: 8 }}>
                  {members.map(m => (
                    <div key={m.user_key} onClick={() => openColleagueProfile(m.user_key)} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: '8px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        {canManageMembers && m.user_key !== selectedRoom.created_by && (
                          <button className="btn-admin btn-danger" style={{ padding: '3px 8px', fontSize: 11 }} onClick={e => { e.stopPropagation(); removeMember(m.user_key); }}>Remover</button>
                        )}
                      </div>
                    </div>
                  ))}

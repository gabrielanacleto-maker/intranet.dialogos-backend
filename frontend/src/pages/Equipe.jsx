...
function OrgCard({ u, tier }) {
  ...
  const tenureClass = getTenureClass(u.hire_date);
  return (
    <div className={`org-card tier-${tier}`} onClick={() => openColleagueProfile(u.key)} style={{ cursor: 'pointer' }}>
...
function TeamCard({ u, canManage, onEdit }) {
  ...
  const tenureClass = getTenureClass(u.hire_date);
  return (
    <div className="team-card" onClick={() => openColleagueProfile(u.key)} style={{ cursor: 'pointer' }}>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
      ...
      <UserBadges u={u} />
      {canManage && (
        <button className="btn-admin btn-secondary" style={{ marginTop: 10, padding: '4px 10px', fontSize: 11 }} onClick={e => { e.stopPropagation(); onEdit(); }}>✏️ Editar</button>
      )}
    </div>
  );
}

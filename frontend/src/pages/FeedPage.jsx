...
import { AV_COLORS, timeAgo, buildEmbedHtml, getRoleStyle, getRoleGlowClass, ALLOWED_VIDEO_TYPES } from '../utils';
import MoodWidget from '../components/MoodWidget';
import { openColleagueProfile } from './ColleagueProfilePage';
import { GifPicker, MentionSuggest, renderTextMedia } from '../components/TextTools';

const BASE = import.meta.env.VITE_API_URL || 'http://localhost:8000';

// ── Frontend sanitization helper (defense in depth — never trust input) ─────
...
// ── Role glow helper (centralized in utils) ───────────────────────────────────

// ── Avatar helper ────────────────────────────────────────────────────────────
function Avatar({ photoUrl, initials, color, size = 36, onClick }) {
  ...
  return (
    <button type="button" onClick={onClick} style={{
      ...
      flexShrink: 0, overflow: 'hidden', border: 0, padding: 0,
...
export default function FeedPage({ feedType = 'feed' }) {
  ...
  // is_leader inherits equivalent permissions to is_diretor where applicable
  const canMarkDiretoria = user?.role === 'Diretora' || user?.role === 'Diretor' || user?.is_diretor || user?.is_leader;
const canMarkLideranca = user?.role === 'Líder' || user?.is_leader;
...
useEffect(() => {
  ...
  async function handleVideoSelect(e) {
    const file = e.target.files[0];
    if (!file) return;
    if (!ALLOWED_VIDEO_TYPES.includes(file.type)) return toast('Formato de vídeo não suportado. Use MP4, MOV ou WEBM.', 'error');
    if (file.size > 50 * 1024 * 1024) return toast('Vídeo muito grande (máx 50MB).', 'error');
    setUploadingVideo(true);
    try {
      const localUrl = URL.createObjectURL(file);
      setVideoUrl({ local: localUrl, file });
      toast('Vídeo selecionado! Faça upload antes de publicar.', 'info');
    } catch (err) { toast(err.message, 'error'); }
  ...
  async function uploadVideo() {
    ...
    setUploadingVideo(true);
    try {
      const res = await api.uploadPostVideo(videoUrl.file);
      setVideoUrl({ ...videoUrl, remote: res.url });
      toast('Vídeo carregado!', 'success');
    } catch (err) { toast(err.message, 'error'); }
    finally { setUploadingVideo(false); }
  }

  async function publish() {
    if (!text.trim() && !imageUrl && !embedUrl.trim() && !videoUrl?.remote) return toast('Escreva algo, adicione imagem, vídeo ou link.', 'error');
    setLoading(true);
    try {
      await api.createPost({
        ...
        image_url: imageUrl,
        video_url: videoUrl?.remote || null,
        embed_url: embedUrl,
        access_level: accessLevel,
        comunicado_tipo: comunicadoTipo !== 'none' ? comunicadoTipo : null,
      });
      setText(''); setImageUrl(null); setVideoUrl(null); setEmbedUrl(''); setShowEmbed(false); setComunicadoTipo('none');
      if (videoUrl?.local) URL.revokeObjectURL(videoUrl.local);
  ...
  async function handleComment(postId) {
    const txt = sanitizeText(commentText[postId]);
    if (!txt) return;
    try {
  ...
  async function handleReply(postId, parentComment) {
    const txt = sanitizeText(replyText[parentComment.id]);
    if (!txt) return;
  ...
  return (
    <div>
        ...
        <div className="composer">
          <div className="composer-top" style={{ position: 'relative' }}>
            ...
            <textarea
              ...
              rows={2}
            />
            <MentionSuggest value={text} onChange={setText} users={mentionUsers} inputRef={composerRef} />
            <button
          ...
          {videoUrl?.local && (
            <div style={{ margin: '6px 0 12px 46px' }}>
              <video
                src={videoUrl.local}
                controls
                style={{ maxWidth: 260, maxHeight: 220, borderRadius: 10, border: '1px solid var(--border)', display: 'block', background: '#000' }}
              />
              <div style={{ marginTop: 6, display: 'flex', gap: 6 }}>
                {!videoUrl.remote && (
                  <button className="btn-admin btn-primary" style={{ padding: '6px 10px' }} onClick={uploadVideo} disabled={uploadingVideo}>
                    {uploadingVideo ? 'Enviando...' : 'Fazer upload do vídeo'}
                  </button>
                )}
                {videoUrl.remote && <span style={{ fontSize: 12, color: 'var(--gold)', fontWeight: 600 }}>✓ Vídeo pronto</span>}
                <button className="btn-admin btn-secondary" style={{ padding: '6px 10px' }} onClick={() => { if (videoUrl?.local) URL.revokeObjectURL(videoUrl.local); setVideoUrl(null); }}>
                  Remover vídeo
                </button>
              </div>
            </div>
          ...
          <div className="composer-actions">
            ...
            <label className="btn-admin btn-secondary" style={{ padding: '8px 12px', cursor: 'pointer' }}>
              {uploadingVideo ? '⏳ Vídeo...' : '🎬 Vídeo'}
              <input type="file" accept="video/mp4,video/quicktime,video/webm" hidden ref={videoFileRef} onChange={handleVideoSelect} />
            </label>
            <button className="btn-admin btn-secondary" style={{ padding: '8px 12px' }} onClick={() => setShowEmbed(!showEmbed)}>
              🔗 Embed / Link Rede Social
            </button>
            <GifPicker onSelect={url => setText(t => `${t}${t ? '\n' : ''}${url}`)} />
      ...
      {posts.map(post => {
        ...
        const roleGlow = getRoleGlowClass(post.author_role, '');
        ...
        return (
          <div key={post.id} className={`post-card${post.pinned ? ' pinned' : ''}`} style={comunicadoStyle ? {
            ...
            <div className="post-header">
              <Avatar photoUrl={post.author_photo_url} initials={post.author_initials} color={post.author_color} size={38} onClick={() => openColleagueProfile(post.author_key)} />
              <div className="post-meta">
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                  <button type="button" className="post-author" onClick={() => openColleagueProfile(post.author_key)} style={{ background: 'none', border: 0, padding: 0, cursor: 'pointer', color: 'inherit', fontFamily: 'inherit' }}>{post.author_name}</button>
                  {post.author_role && (
                    <span className={roleGlow} style={{ fontSize: 11, ...roleStyle }}>
                      {post.author_role}
                    </span>
                  )}
                </div>
            ...
            {post.text && <div className="post-body">{renderTextMedia(post.text)}</div>}
            ...
            {post.video_url && (
              <div className="post-image-wrap">
                <video
                  src={api.assetUrl(post.video_url)}
                  controls
                  preload="metadata"
            ...
            {showComments && (
              <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
                {comments.map(c => {
                  ...
                  const cRoleGlow = getRoleGlowClass(authorRole, '');

                  return (
                    <div key={c.id} style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'flex-start' }}>
                      <Avatar photoUrl={c.author_photo_url} initials={c.author_initials} color={c.author_color} size={30} onClick={() => openColleagueProfile(c.author_key)} />
                      <div style={{ flex: 1 }}>
                        <div style={{ background: 'var(--surface)', borderRadius: 14, padding: '10px 14px', border: '1px solid var(--border)', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 2, flexWrap: 'wrap' }}>
                            <button type="button" onClick={() => openColleagueProfile(c.author_key)} style={{ fontSize: 13, fontWeight: 700, background: 'none', border: 0, color: 'inherit', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>{c.author_name}</button>
                            {authorRole && (
                              <span className={cRoleGlow} style={{ fontSize: 11, ...cRoleStyle }}>{authorRole}</span>
                            )}
                          </div>
                          <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.5 }}>{renderTextMedia(c.text)}</div>
                        </div>
                          ...
                          <div style={{ display: 'flex', gap: 6, marginTop: 6, alignItems: 'center', position: 'relative' }}>
                            ...
                            <div style={{ flex: 1, position: 'relative' }}>
                              <input
                                ...
                                style={{ width: '100%', padding: '6px 32px 6px 12px', borderRadius: 10, border: '1.5px solid var(--border)',
                                  fontSize: 12, fontFamily: 'inherit', background: 'var(--surface)', color: 'var(--text)', boxSizing: 'border-box' }}
                              />
                              <MentionSuggest value={replyText[c.id] || ''} onChange={value => setReplyText(r => ({ ...r, [c.id]: value }))} users={mentionUsers} />
                            ...
                            >Enviar</button>
                            <GifPicker onSelect={url => setReplyText(r => ({ ...r, [c.id]: `${r[c.id] || ''}${r[c.id] ? '\n' : ''}${url}` }))} />
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center', position: 'relative' }}>
                  ...
                  <div style={{ flex: 1, position: 'relative' }}>
                    <input
                      ...
                      style={{ width: '100%', padding: '8px 36px 8px 14px', borderRadius: 12, border: '1.5px solid var(--border)', fontSize: 13, fontFamily: 'inherit', background: 'var(--surface)', color: 'var(--text)', boxSizing: 'border-box' }}
                    />
                    <MentionSuggest value={commentText[post.id] || ''} onChange={value => setCommentText(c => ({ ...c, [post.id]: value }))} users={mentionUsers} />
                  ...
                  <GifPicker onSelect={url => setCommentText(c => ({ ...c, [post.id]: `${c[post.id] || ''}${c[post.id] ? '\n' : ''}${url}` }))} />
                </div>
              </div>
            )}
          </div>

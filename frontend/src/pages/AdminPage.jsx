...
export default function AdminPage() {
  ...
  function defaultForm() {
    return { key: '', name: '', initials: '', role: '', dept: '', level: 'dourado', color: 'av-gold', access_level: 0, is_admin: false, is_admin_user: false, is_rh: false, is_ouvidor: false, is_diretor: false, is_leader: false, points: 100, password: '', hire_date: '', org_position: 'colaborador', is_orcoma: false };
  }
  ...
  function openEdit(u) {
    ...
    setForm({
      ...
      is_diretor: !!u.is_diretor, is_leader: !!u.is_leader,
                    ...
                    <tr key={u.key}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <button type="button" onClick={() => openColleagueProfile(u.key)} style={{ width: 34, height: 34, borderRadius: '50%', background: avatarBg, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, overflow: 'hidden', flexShrink: 0, border: 0, padding: 0, cursor: 'pointer' }}>
                            {u.photo_url ? <img src={api.assetUrl(u.photo_url)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : u.initials}
                          </button>
                          <div>
                            <button type="button" onClick={() => openColleagueProfile(u.key)} style={{ fontWeight: 600, fontSize: 13, background: 'none', border: 0, color: 'inherit', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>{u.name}</button>
                          </div>
                        </div>
                      </td>
            ...
            <div className="admin-form">
              ...
              <div className="admin-field">
                ...
                {[
                  ...
                  { key: 'is_diretor', label: 'Permissão Diretoria — publicar Comunicado Direção' },
                  { key: 'is_leader', label: 'Permissão Liderança — publicar Comunicado Líder' },
                  { key: 'is_admin_user', label: 'Admin Usuário — gerencia colaboradores e pastas', disabled: !isLevel3 },
                  { key: 'is_admin', label: 'Super Admin — acesso total ao sistema', disabled: !isLevel3 },

...
function playNotifSound(soundEnabled) {
  ...
  try {
    ...
    master.gain.linearRampToValueAtTime(0.15, ctx.currentTime + 0.005);
    master.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 1.2);

    // Harmônicos de um sino real (fundamental + parciais)
    const harmonics = [
      { freq: 523.25, gain: 1.0, type: 'sine' },   // C5
      { freq: 1046.5, gain: 0.5, type: 'sine' },    // C6
      { freq: 1568.0, gain: 0.3, type: 'sine' },    // G6
      { freq: 2093.0, gain: 0.15, type: 'sine' },   // C7
    ];
    ...
    harmonics.forEach(({ freq, gain: hGain, type }) => {
      ...
      g.gain.setValueAtTime(hGain * 0.12, ctx.currentTime);
...
export default function NotificationBell() {
  const { user } = useAuth();
  const soundKey = `dialogos_notif_sound_${user?.key || 'anon'}`;
  const [open, setOpen]           = useState(false);
  const [notifs, setNotifs]       = useState([]);
  const [unread, setUnread]       = useState(0);
  const [loading, setLoading]     = useState(false);
  const [soundOn, setSoundOn]     = useState(
    () => localStorage.getItem(`dialogos_notif_sound_${user?.key || 'anon'}`) !== 'false'
  );
  ...
  const fetchCount = useCallback(async () => {
    try {
      ...
      if (prevUnread.current !== null && count > prevUnread.current) playNotifSound(soundOn);

...
r = client.post("/api/dart/avaliacoes", json={"avaliando_id": AVA}, headers=H)
print("CREATE:", r.status_code, json.dumps(r.json(), ensure_ascii=False))

if r.status_code == 200:
    data = r.json()
    token = data["link"].split("/")[-1]
    link = "http://localhost:8000/dart/avaliacao/" + token
    print("LINK DE TESTE:", link)

    # public info to confirm it's loadable
    info = client.get(f"/api/public/dart/{token}/info")
    ...
    q = client.get(f"/api/public/dart/{token}/perguntas").json()["perguntas"]
    ...
    sub = client.post(f"/api/public/dart/{token}/respostas", json={"respostas": payload})
    print("SUBMIT:", sub.status_code, json.dumps(sub.json(), ensure_ascii=False))
    det = client.get(f"/api/dart/avaliacoes/{data['id']}", headers=H).json()
